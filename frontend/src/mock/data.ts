import type { CognateSet, LanguageFamily, WordNode, WordLink } from '../types'
...
export function rootNodeId(index: number): string {
  return 'root_' + index
}

export function wordNodeId(index: number, language: string): string {
  return index + '_' + language
}
